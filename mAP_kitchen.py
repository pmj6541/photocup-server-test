...
for i in range(50):
    if i < 10 :
        ...
        filename = f"{i}.png"
    img = cv2.imread("./YoloTest/kitchen/" + filename)
    imgList.append(img)
...
for i in range(50):
    if i < 10 :
        ...
        filename = f"{i}.png"
    img = cv2.imread("./YoloTest/food/" + filename)
    imgList.append(img)
...
for k in range(9):
    ...
    for i in range(len(detectedInfos)) :
        ...
        if 'kitchen' in check_class and i < 50 :
            tp = tp + 1
        elif i < 50 :
            fn = fn + 1
        elif 'kitchen' in check_class and i >= 50 :
            fp = fp + 1
    precision = tp / (tp + fp)
    recall = tp / (tp + fn)
