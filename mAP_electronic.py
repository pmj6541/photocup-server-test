...
for i in range(50):
    if i < 10 :
        ...
        filename = f"{i}.png"
    img = cv2.imread("./YoloTest/electronic/" + filename)
    imgList.append(img)
...
for i in range(50):
    if i < 10 :
        ...
        filename = f"{i}.png"
    img = cv2.imread("./YoloTest/tools/" + filename)
    imgList.append(img)
...
for k in range(9):
    ...
    for i in range(len(detectedInfos)) :
        ...
        if 'electronic' in check_class and i < 50 :
            tp = tp + 1
        elif i < 50 :
            fn = fn + 1
        elif 'electronic' in check_class and i >= 50 :
            fp = fp + 1
    precision = tp / (tp + fp)
    recall = tp / (tp + fn)
