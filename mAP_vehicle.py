...
for i in range(50):
    if i < 10 :
        ...
        filename = f"{i}.png"
    img = cv2.imread("./YoloTest/vehicle/" + filename)
    imgList.append(img)
...
for i in range(50):
    if i < 10 :
        ...
        filename = f"{i}.png"
    img = cv2.imread("./YoloTest/car/" + filename)
    imgList.append(img)
...
for k in range(9):
    ...
    for i in range(len(detectedInfos)) :
        ...
        if 'vehicle' in check_class and i < 50 :
            tp = tp + 1
        elif i < 50 :
            fn = fn + 1
        elif 'vehicle' in check_class and i >= 50 :
            fp = fp + 1
    precision = tp / (tp + fp)
    recall = tp / (tp + fn)
